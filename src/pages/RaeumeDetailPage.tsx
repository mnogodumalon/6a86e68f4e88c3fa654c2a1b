...
export default function RaeumeDetailPage() {
  ...
  const [record, setRecord] = useState<Raeume | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getRaeume();
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteRaeumeEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/raeume');
  }
