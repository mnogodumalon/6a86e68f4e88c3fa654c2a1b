...
import { GeraeteverwaltungDialog } from '@/components/dialogs/GeraeteverwaltungDialog';
import { GeraeteverwaltungViewDialog } from '@/components/dialogs/GeraeteverwaltungViewDialog';
...
const MITGLIEDSBEITRAEGE_FIELDS = [
  { key: 'betrag', label: 'Betrag in Euro', type: 'number' },
  { key: 'zahlungsdatum', label: 'Zahlungsdatum', type: 'date/date' },
  { key: 'mitglieder', label: 'Mitglieder', type: 'multipleapplookup/select', targetEntity: 'mitglieder', targetAppId: 'MITGLIEDER', displayField: 'vorname' },
  { key: 'zahlungsstatus', label: 'Zahlungsstatus', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'bezahlt', label: 'Bezahlt' }, { key: 'erlassen', label: 'Erlassen' }] },
];
const MITGLIEDER_FIELDS = [
  ...
  { key: 'beitrittsdatum', label: 'Beitrittsdatum', type: 'date/date' },
  { key: 'mitgliedsstatus', label: 'Status', type: 'lookup/select', options: [{ key: 'aktiv', label: 'Aktiv' }, { key: 'passiv', label: 'Passiv' }, { key: 'ehrenmitglied', label: 'Ehrenmitglied' }, { key: 'ausgetreten', label: 'Ausgetreten' }] },
  { key: 'telefonnummer', label: 'Telefonnummer', type: 'string/tel' },
];
const GERAETEVERWALTUNG_FIELDS = [
  { key: 'geraetename', label: 'Gerätename', type: 'string/text' },
  { key: 'anschaffungsdatum', label: 'Anschaffungsdatum', type: 'date/date' },
  { key: 'zustand', label: 'Zustand', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'gut', label: 'Gut' }, { key: 'reparaturbeduerftig', label: 'Reparaturbedürftig' }] },
];
const RAEUME_FIELDS = [
  { key: 'raumname', label: 'Raumname', type: 'string/text' },
  { key: 'kapazitaet', label: 'Kapazität (Personen)', type: 'string/text' },
  { key: 'ausstattung', label: 'Ausstattung', type: 'string/textarea' },
  { key: 'stockwerk', label: 'Stockwerk', type: 'string/text' },
];
...
const ENTITY_TABS = [
  { key: 'mitgliedsbeitraege', pascal: 'Mitgliedsbeitraege' },
  { key: 'mitglieder', pascal: 'Mitglieder' },
  { key: 'geraeteverwaltung', pascal: 'Geraeteverwaltung' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('mitgliedsbeitraege');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'mitgliedsbeitraege': new Set(),
    'mitglieder': new Set(),
    'geraeteverwaltung': new Set(),
    'raeume': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'geraeteverwaltung': return (data as any).geraeteverwaltung as Geraeteverwaltung[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mitgliedsbeitraege' && fieldKey === 'mitglieder') {
      const match = (lists.mitgliederList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'veranstaltungen' && fieldKey === 'raum') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'mitgliedsbeitraege': return MITGLIEDSBEITRAEGE_FIELDS as any[];
        case 'mitglieder': return MITGLIEDER_FIELDS as any[];
        case 'geraeteverwaltung': return GERAETEVERWALTUNG_FIELDS as any[];
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'geraeteverwaltung': return {
        create: (fields: any) => LivingAppsService.createGeraeteverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGeraeteverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteGeraeteverwaltungEntry(id),
      };
      case 'raeume': return {
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'geraeteverwaltung' || dialogState?.entity === 'geraeteverwaltung') && (
        <GeraeteverwaltungDialog
          open={createEntity === 'geraeteverwaltung' || dialogState?.entity === 'geraeteverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'geraeteverwaltung' ? handleUpdate : (fields: any) => handleCreate('geraeteverwaltung', fields)}
          defaultValues={dialogState?.entity === 'geraeteverwaltung' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'geraeteverwaltung' && (
        <GeraeteverwaltungViewDialog
          open={viewState?.entity === 'geraeteverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'geraeteverwaltung', record: r }); }}
        />
      )}
