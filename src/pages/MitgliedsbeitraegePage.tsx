...
export default function MitgliedsbeitraegePage() {
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('mitgliedsbeitraege') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('mitgliedsbeitraege') })}
          value={search}
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('betrag')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitgliedsbeitraege', 'betrag')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zahlungsdatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitgliedsbeitraege', 'zahlungsdatum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mitglieder')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitgliedsbeitraege', 'mitglieder')}
                  {sortKey === 'mitglieder' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zahlungsstatus')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitgliedsbeitraege', 'zahlungsstatus')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitgliedsbeitraege/${record.record_id}`); }}>
                ...
                <TableCell>{Array.isArray(record.fields.mitglieder) && record.fields.mitglieder.length > 0 ? <div className="flex flex-wrap gap-1">{record.fields.mitglieder.map((url: any, i: number) => <span key={i} className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getMitgliederDisplayName(url)}</span>)}</div> : '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('mitgliedsbeitraege', 'zahlungsstatus', record.fields.zahlungsstatus?.key) ?? record.fields.zahlungsstatus?.label ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('mitgliedsbeitraege') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
