...
import type { Mitgliedsbeitraege, Mitglieder, Geraeteverwaltung, Raeume, Veranstaltungen, CreateMitgliedsbeitraege, CreateMitglieder, CreateGeraeteverwaltung, CreateRaeume, CreateVeranstaltungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMitgliedsbeitraege(): Promise<Mitgliedsbeitraege[]> {
    ...
    return hydrateRecords(records, 'mitgliedsbeitraege');
  }
  static async getMitgliedsbeitraegeEntry(id: string): Promise<Mitgliedsbeitraege | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Mitgliedsbeitraege;
    return hydrateRecords([record], 'mitgliedsbeitraege')[0];
  }
  static async createMitgliedsbeitraegeEntry(fields: CreateMitgliedsbeitraege): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'mitgliedsbeitraege') });
    return { ...data, record_id: data.id };
  }
  static async updateMitgliedsbeitraegeEntry(id: string, fields: Partial<CreateMitgliedsbeitraege>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitgliedsbeitraege') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- GERAETEVERWALTUNG ---
  static async getGeraeteverwaltung(): Promise<Geraeteverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERAETEVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Geraeteverwaltung[];
    return hydrateRecords(records, 'geraeteverwaltung');
  }
  static async getGeraeteverwaltungEntry(id: string): Promise<Geraeteverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERAETEVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Geraeteverwaltung;
    return hydrateRecords([record], 'geraeteverwaltung')[0];
  }
  static async createGeraeteverwaltungEntry(fields: CreateGeraeteverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.GERAETEVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'geraeteverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateGeraeteverwaltungEntry(id: string, fields: Partial<CreateGeraeteverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.GERAETEVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'geraeteverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteGeraeteverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GERAETEVERWALTUNG}/records/${id}`);
  }
