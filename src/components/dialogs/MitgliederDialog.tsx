...
export type MitgliederDialogDefaults = Omit<Mitglieder['fields'], 'mitgliedsstatus'> & {
...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail-Adresse\n  "beitrittsdatum": string | null, // YYYY-MM-DD\n  "mitgliedsstatus": LookupValue | null, // Status (select one key: "aktiv" | "passiv" | "ehrenmitglied" | "ausgetreten") mapping: aktiv=Aktiv, passiv=Passiv, ehrenmitglied=Ehrenmitglied, ausgetreten=Ausgetreten\n  "telefonnummer": string | null, // Telefonnummer\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beitrittsdatum': (
      <div key="beitrittsdatum" className="space-y-1.5">
        <Label htmlFor="beitrittsdatum">{fieldLabel('mitglieder', 'beitrittsdatum')}</Label>
        <DatePicker
          id="beitrittsdatum"
          placeholder="Wann beigetreten?"
          mode="date"
          value={fields.beitrittsdatum ?? null}
          onChange={v => setFields(f => ({ ...f, beitrittsdatum: v ?? undefined }))}
        />
      </div>
    ),
    'mitgliedsstatus': (
      <div key="mitgliedsstatus" className="space-y-1.5">
        <Label htmlFor="mitgliedsstatus">{fieldLabel('mitglieder', 'mitgliedsstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitglieder', 'mitgliedsstatus', 'aktiv') ?? 'Aktiv'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitglieder', 'mitgliedsstatus', 'passiv') ?? 'Passiv'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitglieder', 'mitgliedsstatus', 'ehrenmitglied') ?? 'Ehrenmitglied'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitglieder', 'mitgliedsstatus', 'ausgetreten') ?? 'Ausgetreten'}
          </button>
        </div>
      </div>
    ),
    'telefonnummer': (
      <div key="telefonnummer" className="space-y-1.5">
        <Label htmlFor="telefonnummer">{fieldLabel('mitglieder', 'telefonnummer')}</Label>
        <Input
          id="telefonnummer"
          value={fields.telefonnummer ?? ''}
          onChange={e => setFields(f => ({ ...f, telefonnummer: e.target.value }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "beitrittsdatum": "Beitrittsdatum", "mitgliedsstatus": "Status", "telefonnummer": "Telefonnummer"};
