 ...
 * raeumeList (full hook array — resolves the Raeume applookup),
...
export type VeranstaltungenDialogDefaults = Veranstaltungen['fields'];
...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, raeumeList, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  // Inline-Create state for "Raeume" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraRaeume` list, and select it in
  // the originating Combobox via the captured `createRaeumeField`.
  const [createRaeumeOpen, setCreateRaeumeOpen] = useState(false);
  const [createRaeumeInitial, setCreateRaeumeInitial] = useState('');
  const [createRaeumeField, setCreateRaeumeField] = useState<string>('');
  const [extraRaeume, setExtraRaeume] = useState< Raeume[]>([]);
  const raeumeListAll = useMemo(
    () => [...raeumeList, ...extraRaeume],
    [raeumeList, extraRaeume],
  );
  function openCreateRaeume(fieldKey: string, q: string) {
    setCreateRaeumeField(fieldKey);
    setCreateRaeumeInitial(q);
    setCreateRaeumeOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['titel', 'datum'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="raum" entity="Räume">\n${JSON.stringify(raeumeList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["raum"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Agenda, Inhalte, Besonderheiten..."
    ...
    'raum': (
      <div key="raum" className="space-y-1.5">
        <Label htmlFor="raum">{fieldLabel('veranstaltungen', 'raum')}</Label>
        <Combobox
          id="raum"
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "datum": "Datum und Uhrzeit", "beschreibung": "Beschreibung", "raum": "Raum"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"raum": {"raumname": "Raumname", "kapazitaet": "Kapazität (Personen)", "ausstattung": "Ausstattung", "stockwerk": "Stockwerk"}};
  ...
  return (
    ...
    {createRaeumeOpen && (
      <RaeumeDialog
        open={createRaeumeOpen}
        onClose={() => setCreateRaeumeOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createRaeumeEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Raeume;
            setExtraRaeume(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.RAEUME, result.id);
            setFields(prev => ({ ...prev, [createRaeumeField]: url } as any));
          }
          setCreateRaeumeOpen(false);
        }}
        defaultValues={createRaeumeInitial
          ? ({ raumname: createRaeumeInitial } as any)
          : undefined}
      />
    )}
    </>
