/**
 * GeraeteverwaltungDialog — pre-generated create/edit dialog for Geraeteverwaltung.
 *
 ...
 * GeraeteverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Geraeteverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<GeraeteverwaltungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Geraeteverwaltung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Geraeteverwaltung';
...
/** Widened prefill type for GeraeteverwaltungDialog.defaultValues — see file header. */
export type GeraeteverwaltungDialogDefaults = Omit<Geraeteverwaltung['fields'], 'zustand'> & {
    zustand?: LookupValue | string;
  };

interface GeraeteverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Geraeteverwaltung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: GeraeteverwaltungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zustand: LOOKUP_OPTIONS['geraeteverwaltung']?.['zustand'] ?? [],
};
...
export function GeraeteverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: GeraeteverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Geraeteverwaltung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['geraetename'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Geraeteverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'geraeteverwaltung');
      await onSubmit(clean as Geraeteverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "geraetename": string | null, // Gerätename\n  "anschaffungsdatum": string | null, // YYYY-MM-DD\n  "zustand": LookupValue | null, // Zustand (select one key: "neu" | "gut" | "reparaturbeduerftig") mapping: neu=Neu, gut=Gut, reparaturbeduerftig=Reparaturbedürftig\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Geraeteverwaltung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('geraeteverwaltung') })
    : t('new_entity', { entity: appLabel('geraeteverwaltung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'geraetename': (
      <div key="geraetename" className="space-y-1.5">
        <Label htmlFor="geraetename">{fieldLabel('geraeteverwaltung', 'geraetename')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="geraetename"
          placeholder="z. B. Beamer, Flipchart"
          value={fields.geraetename ?? ''}
          onChange={e => setFields(f => ({ ...f, geraetename: e.target.value }))}
          required
        />
        {showErrors && !fields.geraetename && (
    ...
    'anschaffungsdatum': (
      <div key="anschaffungsdatum" className="space-y-1.5">
        <Label htmlFor="anschaffungsdatum">{fieldLabel('geraeteverwaltung', 'anschaffungsdatum')}</Label>
        <DatePicker
    ...
    'zustand': (
      <div key="zustand" className="space-y-1.5">
        <Label htmlFor="zustand">{fieldLabel('geraeteverwaltung', 'zustand')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('geraeteverwaltung', 'zustand', 'neu') ?? 'Neu'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('geraeteverwaltung', 'zustand', 'gut') ?? 'Gut'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('geraeteverwaltung', 'zustand', 'reparaturbeduerftig') ?? 'Reparaturbedürftig'}
          </button>
        </div>
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"geraetename": "Gerätename", "anschaffungsdatum": "Anschaffungsdatum", "zustand": "Zustand"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.GERAETEVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
