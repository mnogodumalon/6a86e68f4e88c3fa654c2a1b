/**
 * MitgliedsbeitraegeDialog — pre-generated create/edit dialog for Mitgliedsbeitraege.
 *
 ...
 * MitgliedsbeitraegeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Mitgliedsbeitraege['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<MitgliedsbeitraegeDialogDefaults | undefined>(undefined)
 */
...
/** Widened prefill type for MitgliedsbeitraegeDialog.defaultValues — see file header. */
export type MitgliedsbeitraegeDialogDefaults = Omit<Mitgliedsbeitraege['fields'], 'zahlungsstatus'> & {
...
interface MitgliedsbeitraegeDialogProps {
  ...
  defaultValues?: MitgliedsbeitraegeDialogDefaults;
...
export function MitgliedsbeitraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: MitgliedsbeitraegeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['betrag', 'zahlungsdatum', 'mitglieder'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "betrag": number | null, // Betrag in Euro\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "mitglieder": string[] | null, // Display names from Mitglieder, one per referenced record (see <available-records>)\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "offen" | "bezahlt" | "erlassen") mapping: offen=Offen, bezahlt=Bezahlt, erlassen=Erlassen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitglieder"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const mitgliederNames = raw['mitglieder'];
        if (Array.isArray(mitgliederNames) && mitgliederNames.length > 0) {
          const mitgliederUrls = (mitgliederNames as unknown[])
          ...
          if (mitgliederUrls.length > 0) merged['mitglieder'] = mitgliederUrls;
        }
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('mitgliedsbeitraege') })
    : t('new_entity', { entity: appLabel('mitgliedsbeitraege') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'betrag': (
      <div key="betrag" className="space-y-1.5">
        <Label htmlFor="betrag">{fieldLabel('mitgliedsbeitraege', 'betrag')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="betrag"
          type="number"
          step="any"
    ...
    'zahlungsdatum': (
      <div key="zahlungsdatum" className="space-y-1.5">
        <Label htmlFor="zahlungsdatum">{fieldLabel('mitgliedsbeitraege', 'zahlungsdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="zahlungsdatum"
    ...
    'mitglieder': (
      <div key="mitglieder" className="space-y-1.5">
        <Label htmlFor="mitglieder">{fieldLabel('mitgliedsbeitraege', 'mitglieder')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <MultiCombobox
          id="mitglieder"
          placeholder="Welche Mitglieder?"
          items={mitgliederListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          values={extractRecordIds(fields.mitglieder)}
          onChange={ids => setFields(f => ({ ...f, mitglieder: ids.length ? ids.map(id => createRecordUrl(APP_IDS.MITGLIEDER, id)) as any : undefined }))}
          onCreateNew={(q) => openCreateMitglieder("mitglieder", q)}
          createLabel={t('create_in', { entity: appLabel('mitglieder') })}
        />
        {showErrors && !fields.mitglieder && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'zahlungsstatus': (
      <div key="zahlungsstatus" className="space-y-1.5">
        <Label htmlFor="zahlungsstatus">{fieldLabel('mitgliedsbeitraege', 'zahlungsstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitgliedsbeitraege', 'zahlungsstatus', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitgliedsbeitraege', 'zahlungsstatus', 'bezahlt') ?? 'Bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsstatus) === 'erlassen'}
            onClick={() => setFields(f => ({ ...f, zahlungsstatus: (lookupKey(f.zahlungsstatus) === 'erlassen' ? undefined : 'erlassen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsstatus) === 'erlassen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitgliedsbeitraege', 'zahlungsstatus', 'erlassen') ?? 'Erlassen'}
          </button>
        </div>
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"betrag": "Betrag in Euro", "zahlungsdatum": "Zahlungsdatum", "mitglieder": "Mitglieder", "zahlungsstatus": "Zahlungsstatus"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitglieder": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "beitrittsdatum": "Beitrittsdatum", "mitgliedsstatus": "Status", "telefonnummer": "Telefonnummer"}};
