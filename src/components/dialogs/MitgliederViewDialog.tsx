    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('mitglieder', 'beitrittsdatum')}</Label>
            <p className="text-sm">{formatDate(record.fields.beitrittsdatum)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('mitglieder', 'mitgliedsstatus')}</Label>
            <Badge variant="secondary">{lookupLabel('mitglieder', 'mitgliedsstatus', record.fields.mitgliedsstatus?.key) ?? record.fields.mitgliedsstatus?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('mitglieder', 'telefonnummer')}</Label>
