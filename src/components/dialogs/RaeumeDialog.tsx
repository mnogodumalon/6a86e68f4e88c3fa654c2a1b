/**
 * RaeumeDialog — pre-generated create/edit dialog for Raeume.
 *
 ...
 * RaeumeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Raeume['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<RaeumeDialogDefaults | undefined>(undefined)
 */
...
/** Widened prefill type for RaeumeDialog.defaultValues — see file header. */
export type RaeumeDialogDefaults = Raeume['fields'];
...
interface RaeumeDialogProps {
  ...
  defaultValues?: RaeumeDialogDefaults;
...
export function RaeumeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: RaeumeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['raumname'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Raeume['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'raeume');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "raumname": string | null, // Raumname\n  "kapazitaet": string | null, // Kapazität (Personen)\n  "ausstattung": string | null, // Ausstattung\n  "stockwerk": string | null, // Stockwerk\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'raumname': (
      <div key="raumname" className="space-y-1.5">
        ...
        <Input
          id="raumname"
          placeholder="z. B. Konferenzsaal A"
    ...
    'kapazitaet': (
      <div key="kapazitaet" className="space-y-1.5">
        ...
        <Input
          ...
          onChange={e => setFields(f => ({ ...f, kapazitaet: e.target.value }))}
        />
      </div>
    ),
    'ausstattung': (
      <div key="ausstattung" className="space-y-1.5">
        ...
        <Textarea
          id="ausstattung"
          placeholder="Beamer, Whiteboard, Tische..."
          value={fields.ausstattung ?? ''}
          onChange={e => setFields(f => ({ ...f, ausstattung: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'stockwerk': (
      <div key="stockwerk" className="space-y-1.5">
        <Label htmlFor="stockwerk">{fieldLabel('raeume', 'stockwerk')}</Label>
        <Input
          id="stockwerk"
          placeholder="z. B. 1, EG, 2. OG"
          value={fields.stockwerk ?? ''}
          onChange={e => setFields(f => ({ ...f, stockwerk: e.target.value }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"raumname": "Raumname", "kapazitaet": "Kapazität (Personen)", "ausstattung": "Ausstattung", "stockwerk": "Stockwerk"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.RAEUME} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
